...
public class BPlusTree implements Iterable<List<KVPair>> {
...
//                    " for key: " + l.getKeyAt(index));
    ...
    private void parseOverflowPages(TreeLeaf l, int index, RangeResult res)
        ...
        while(icap < ovfPage.getCurrentCapacity()) {
            res.getQueryResult().add(new KVPair(l.getKeyAt(index),
        ...
        if(sMin.isFound()) {
            ...
            while(sMin.getLeaf().getKeyAt(i) <= maxKey) {
                rangeQueryResult.getQueryResult().
                        add(new KVPair(sMin.getLeaf().getKeyAt(i),
...
//    @SuppressWarnings("unused")
    ...
    public Iterator<List<KVPair>> iterator() {
        return new Iterator<List<KVPair>>() {

//            /**
//             * 单个节点遍历B+树
//             */
//            private int cur=0, overCur=-1;
//            private boolean hasInit = false;
//            //            private TreeLeaf backNode;
//            private TreeLeaf node;
//            private TreeOverflow overNode = null;
//
//            public boolean hasNext() {
//                if (!hasInit) {
//                    if (root.isInternalNode()) {
//                        node = findLeftLeaf(root);
//                    } else node = (TreeLeaf) root;
////                    hasInit = true;
//                    return true;
//                }
//                // 正常node的下一指针为空 且遍历到最后一个 且如果最后一个有溢出页 也是最后一个
//                if (node.getNextPagePointer() == -1 && (cur + 1 == node.keyArray.size())) {
//                    if (node.overflowList.get(cur) == -1) return false;
//                    if (overNode != null && node.overflowList.get(cur) != -1 && overCur + 1 == overNode.valueList.size()) {
//                        return false;
//                    }
//                }
//                return true;
//            }
//
//            public KeyValueWrapper next() {
//                if (!hasInit) {
//                    hasInit = true;
//                    return new KeyValueWrapper(node.getKeyAt(cur), node.valueList.get(cur));
//                }
//                if (!hasNext()) {
//                    throw new NoSuchElementException("No more elements in the tree");
//                }
//                // 不在溢出页，且有溢出页，则遍历溢出页
//                if (node.overflowList.get(cur) != -1) {
//                    overCur++;
//                    // 初始化溢出页
//                    if (overCur == 0) {
//                        try {
//                            overNode = (TreeOverflow) readNode(node.getOverflowPointerAt(cur));
//                        } catch (IOException e) {
//                            node = null;
//                            return null;
//                        }
//                    }
//                    // 溢出页溢出
//                    assert overNode != null;
//                    if (overCur >= overNode.valueList.size()) {
//                        overCur = -1;
//                        overNode = null;
//                    } else { // 没有溢出，返回溢出页
//                        return new KeyValueWrapper(node.getKeyAt(cur), overNode.getValueAt(overCur));
//                    }
//                }
//                cur++;
//                if (cur >= node.getKeyArrayLen()) { // 超出当前叶节点，前往下一个
//                    try {
//                        node = (TreeLeaf) readNode(node.getNextPagePointer());
//                        cur = 0;
//                    } catch (IOException e) {
//                        node = null;
//                        return null;
...
//                }
//                assert node != null;
//                return new KeyValueWrapper(node.getKeyAt(cur), node.getValueAt(cur));
//            }
//
//
            ...
            public boolean hasNext() {
                ...
                // 正常node的下一指针为空 且遍历到最后一个
                if (node == null || node.getNextPagePointer() == -1 && (cur + 1 == node.keyArray.size())) {
                    return false;
                }
                return true;
            }

            public List<KVPair> next() {
                if (!hasInit) {
                    if (root.isInternalNode()) {
                        node = findLeftLeaf(root);
                    } else node = (TreeLeaf) root;
                ...
                assert node != null;
                res.add(new KVPair(node.getKeyAt(cur), node.valueList.get(cur).trim()));
                if (node.overflowList.get(cur) != -1) {
                    try {
                        TreeOverflow overNode = (TreeOverflow) readNode(node.overflowList.get(cur));
                        while (overNode != null) {
                            for (int i = 0; i < overNode.valueList.size(); i++) {
                                res.add(new KVPair(node.getKeyAt(cur), overNode.valueList.get(i).trim()));
                            }
                            overNode = (TreeOverflow) readNode(overNode.getNextPagePointer());
                        }
                    } catch (IOException e) {
                        return null;
                    }
    ...
    public Map<Long,List<String>> getAllNodes(){
        ...
        for (List<KVPair> kvp : this) {
            res.put(kvp.get(0).getK(), kvp.stream().map(KVPair::getV).collect(Collectors.toList()));
        }
        return res;
    }


    public void printLeafNodes() throws IOException {
        TreeLeaf left = null;
        if (root.isInternalNode()) {
            left = findLeftLeaf(root);
        } else left = (TreeLeaf) root;

        while (left != null) {
            System.out.println("\nNext pointer (index): " + left.getNextPagePointer());
            System.out.println("Prev pointer (index): " + left.getPrevPagePointer());
            System.out.println("Printing stored (Key, Value, ovf) tuples:");
            for (int i = 0; i < left.keyArray.size(); i++) {
                System.out.printf("(%d,%s) ", left.keyArray.get(i), left.valueList.get(i).trim());
                long ovflPtr = left.overflowList.get(i);
                if (ovflPtr != -1) {
                    printOverflow((TreeOverflow) readNode(ovflPtr), left.keyArray.get(i));
                }
            }
            System.out.println();
            left = (TreeLeaf) readNode(left.getNextPagePointer());
        }
    }

    private void printOverflow(TreeOverflow node, Long key) throws IOException {
        while (node != null) {
            for (int i = 0; i < node.valueList.size(); i++) {
                System.out.printf("(%d,%s) ", key, node.valueList.get(i).trim());
            }
            node = (TreeOverflow) readNode(node.getNextPagePointer());
        }
    }

    private TreeLeaf findLeftLeaf(TreeNode node) {
        if (node.isInternalNode()) {
            TreeInternalNode t = (TreeInternalNode) node;
            long ptr = t.getPointerAt(0);
            try {
                return findLeftLeaf(Objects.requireNonNull(readNode(ptr)));
            } catch (IOException e) {
                return null;
            }

        }
        assert node instanceof TreeLeaf;
